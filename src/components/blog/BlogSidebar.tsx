...
const BlogSidebar = ({
  ...
  useEffect(() => {
    ...
    const results = allPosts.filter(post => post.title.toLowerCase().includes(query) || post.excerpt.toLowerCase().includes(query) || post.tags && post.tags.some(tag => tag.toLowerCase().includes(query)));
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm">
        ...
        <div className="relative">
          ...
          <Input placeholder="Buscar por título, contenido o tags..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
        </div>
          ...
          {recentPosts.map(post => (
            <Card key={post.id} className="overflow-hidden">
              ...
              <CardFooter className="pt-0">
                <Button asChild variant="link" className="text-novativa-teal p-0 h-auto text-sm">
